...
fn number_of_bags_in_bag(bag: &Bag, bag_list: &BagList) -> usize {
    bag.containing
        .iter()
        .map(|(capacity, bagcolor)| {
            if let Some(containing_bag) = bag_list.find_by_color(bagcolor) {
                let hold = capacity + capacity * number_of_bags_in_bag(&containing_bag, &bag_list);
                return hold;
            } else {
                return capacity * 1;
            }
...
fn main() {
    ...
    let golden_bag = bags.find_by_color(&String::from("shiny gold")).unwrap();
    println!(
        "gold can hold: {} bags",
        number_of_bags_in_bag(&golden_bag, &bags)
    );
}
