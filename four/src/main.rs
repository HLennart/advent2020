...
// forgot to save #1 beforehand :x

lazy_static! {
    static ref BYR_REGEX: Regex = Regex::new(r"\b(byr:)(\d{4})\b").unwrap();
    static ref IYR_REGEX: Regex = Regex::new(r"\b(iyr:)(\d{4})\b").unwrap();
    static ref EYR_REGEX: Regex = Regex::new(r"\b(eyr:)(\d{4})\b").unwrap();
    static ref HGT_REGEX: Regex = Regex::new(r"\b(hgt:)(\d{3}cm|\d{2}in)\b").unwrap();
    static ref HCL_REGEX: Regex = Regex::new(r"\b(hcl:)#([0-9a-f]{6})\b").unwrap();
    static ref ECL_REGEX: Regex = Regex::new(r"\b(ecl:)(amb|blu|brn|gry|grn|hzl|oth)\b").unwrap();
    static ref PID_REGEX: Regex = Regex::new(r"\b(pid:)(\d{9})\b").unwrap();
}
fn main() {
    let passports: Vec<&str> = INPUT.split("\n\n").collect();

    // should've used a filter
    let number_of_valid_passports = passports
        .iter()
        .map(|&passport| {
            let x = check_passport_valid(passport) as i32;
            if x == 1 {
                println!("{}", passport);
            }
            return x;
        ...
        .sum::<i32>();
    println!("number of valid passports: {}", number_of_valid_passports);
}
...
fn byr_valid(passport: &str) -> bool {
    BYR_REGEX.captures_iter(&passport).any(|byr_sequence| {
        let year = byr_sequence[2].parse::<usize>().expect("wrong year format");
        year >= 1920 && year <= 2002
    })
}

fn iyr_valid(passport: &str) -> bool {
    IYR_REGEX.captures_iter(&passport).any(|iyr_sequence| {
        let year = iyr_sequence[2].parse::<usize>().expect("wrong year format");
        year >= 2010 && year <= 2020
    })
}

fn eyr_valid(passport: &str) -> bool {
    EYR_REGEX.captures_iter(&passport).any(|eyr_sequence| {
        let year = eyr_sequence[2].parse::<usize>().expect("wrong year format");
        year >= 2020 && year <= 2030
    })
}

fn hgt_valid(passport: &str) -> bool {
    HGT_REGEX.captures_iter(&passport).any(|hgt_sequence| {
        if hgt_sequence[2].ends_with("in") {
            let number = hgt_sequence[2][..2]
                .parse::<usize>()
                .expect("cant parse in year");

            return number >= 59 && number <= 76;
        } else if hgt_sequence[2].ends_with("cm") {
            let number = hgt_sequence[2][..3]
                .parse::<usize>()
                .expect("cant parse cm year");
...
const INPUT: &str = "hcl:5d90f0 cid:270 ecl:#66dc9c hgt:62cm byr:1945 pid:63201172 eyr:2026
