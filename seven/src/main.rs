...
lazy_static! {
    static ref re_first: Regex =
        Regex::new(r"(\w+\s\w+) bags contain (\d) (\w+\s\w+) bag[s]?").unwrap();
    static ref re_multiple: Regex = Regex::new(r"(\d) (\w+\s\w+) bag[s]?").unwrap();
}
#[derive(Debug, Clone)]
struct Bag {
    color: String,
    containing: Vec<(Capacity, BagColor)>,
}
...
impl Bag {
    ...
    fn add_bag(&mut self, c: Capacity, b: BagColor) {
        self.containing.push((c, b));
    }

    fn can_hold_golden(&self) -> CanHoldGolden {
        self.containing.iter().any(|(_, b)| b == "shiny gold")
    }
}

type CanHoldGolden = bool;
type Capacity = usize;
...
impl BagList {
    ...
    pub fn push(&mut self, b: Bag) {
        self.list.push(b);
    }

    pub fn find_by_color(&self, b: &BagColor) -> Option<Bag> {
        if let Some(bag) = self.list.iter().find(|bag| bag.color == *b) {
            Some(bag.clone())
        } else {
            None
        }
...
fn can_bag_hold_gold(b: &Bag, list: &BagList) -> bool {
    if b.can_hold_golden() {
        return true;
    }
    let x = b
        .containing
        .iter()
        .map(|(capacity, bag_color)| list.find_by_color(bag_color))
        .collect::<Vec<_>>();
    if x.iter().any(|bag| {
        if let Some(b) = bag {
            if b.can_hold_golden() {
                ...
                b.containing.iter().any(|(_, b)| {
                    if let Some(rec_bag) = list.find_by_color(b) {
                        return can_bag_hold_gold(&rec_bag, list);
                    } else {
                        return false;
                    }
                })
...
fn main() {
    let mut bags = BagList::new();
    INPUT.split("\n").for_each(|row| {
        let x = row.split(",").collect::<Vec<_>>();
        if let Some((first, elements)) = x.split_first() {
            if let Some(captures) = re_first.captures(first) {
                let mut bag = Bag::new(String::from(&captures[1]));
                bag.add_bag(
                    captures[2].parse::<usize>().unwrap(),
                    String::from(&captures[3]),
                );
                for element in elements {
                    for cap in re_multiple.captures_iter(element) {
                        bag.add_bag(cap[1].parse::<usize>().unwrap(), String::from(&cap[2]));
                    }
                }
                bags.push(bag);
            }
    ...
    bags.list.iter().for_each(|bag| {
        println!(
            "{} can it hold golden?: {}",
            bag.color,
            can_bag_hold_gold(bag, &bags)
        );
    });
    ...
    let result = bags
        .list
        .iter()
        .map(|bag| can_bag_hold_gold(bag, &bags) as usize)
        .sum::<usize>();
...
const INPUT: &str = "dark orange bags contain 3 dark chartreuse bags.
