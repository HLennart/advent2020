...
fn solve(input: &str) -> usize {
    input
        ...
        .map(|s| {
            ...
            let begin = splitstring[0];
            let end = splitstring[1];
            let beginsplit: Vec<&str> = begin.split(" ").collect();
            let character = beginsplit[1];
            let rangesplit: Vec<&str> = beginsplit[0].split("-").collect();
            let lower = rangesplit[0].parse::<usize>().unwrap();
            let upper = rangesplit[1].parse::<usize>().unwrap();

            let matches_lower =
                end.chars().nth(lower).unwrap() == character.chars().nth(0).unwrap();
            let matches_upper =
                end.chars().nth(upper).unwrap() == character.chars().nth(0).unwrap();
