fn main() {
    ...
    let result: usize = INPUT
        .split("\n\n")
        .map(|group_result| {
            let group_answers = group_result.split("\n");

            let mut result_all_yes = 0;

            ('a'..='z').for_each(|character| {
                if group_answers
                    // need to clone here, .all seems to somehow mutate the iterator?
                    .clone()
                    .all(|individual| individual.contains(character))
                {
                    result_all_yes += 1;
                }
            });
